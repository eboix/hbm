function [class,V,D] = graph_pow_adj_trunc_weighted_classifier(obj,graph_pow,giant_A,giant_rev)
    ...
    % EDGES.
    % THE SUBSAMPLING IS NOT UNIFORM IN THIS CASE! WEIGHTS ARE ASSIGNED
    % BASED ON THE HERE-TO-FORE CONSIDERED VALUES. ONLY THE HIGHEST ENTRIES
    % ARE KEPT.
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%

    disp('Running graph_pow_adj_trunc_weighted_classifier');
    if nargin == 2
    ...
    for iter = 2:graph_pow
        A_pow = giant_A*A_pow;
        A_pow = triu(A_pow,1);
        [row, col, s]= find(A_pow);
        numnonzero = length(row);
        if e_bound < numnonzero
            % Randomize + sort (so that stability of sort doesn't give
            % unexpected behavior).
           shuffle_perm = randperm(numnonzero);
           s = s(shuffle_perm);
           [~, idx] = sort(s,'descend');
           idx = shuffle_perm(idx);
           idx_remove = idx((e_bound+1):end);
           pos_remove = sub2ind(size(A_pow), row(idx_remove), col(idx_remove));
           A_pow(pos_remove) = 0;
        end
        A_pow = A_pow + A_pow';
    end
