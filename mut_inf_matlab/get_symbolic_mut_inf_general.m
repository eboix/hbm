function mut_inf = get_symbolic_mut_inf_general(n, m, edge_list)
% n = # of vertices.
% m = # of edges.
...
% Returns mut_inf, function of edge_list*2 flip probability variables. Don't plug in 0 or 1 as
% flip probabilities, since you will divide by zero :(. Instead plug in small values to
% simulate the same result.

N = n + m;

errs = sym('errflip',[m 2]);
prob = cell(1,2^(N-1));
for vv=0:2^(n-1)-1,
    ...
    for ee=0:2^m-1
        ...
        for j = 1:size(edge_list,1)
            ...
            if currstat(u) == currstat(v)
                if(currstat(e) == '1'), cp = cp * (1-errs(j,2)); 
                else cp = cp * errs(j,2); end
            else
                if(currstat(e) == '1'), cp = cp * errs(j,1);
                else cp = cp * (1-errs(j,1)); end
            end
        end
